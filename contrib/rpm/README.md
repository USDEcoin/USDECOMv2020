...
The RPM spec file provided here is for Usdecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://usdecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Usdecoin git repository](https://github.com/usdecoin/usdecoin) but are not part of
...
Currently only `Source10` is used, for the example `usdecoin.conf` file.
...
in the usdecoin git repository. At present only an SVG version of the Usdecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the usdecoin
...
needed when 0.12.1 is released, a proper fix is already in the Usdecoin git
master branch.
...
idea to build Usdecoin Core against a static version of that library compiled
...
The `usdecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `usdecoin-server` package. At this point, most RPM based
...
The `usdecoin-server` package should have SELinux support. How to properly do
...
Usdecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/usdecoin/). He has been
packaging Usdecoin for Fedora at least since 2012.
...
4. I renamed the `usdecoin` package that contains the Qt GUI to `usdecoin-core` as
that appears to be how the general population refers to it, in contrast to
`usdecoin-xt` or `usdecoin-classic`. I wanted to make sure the general population
