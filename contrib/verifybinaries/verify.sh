#!/usr/bin/env bash
# Copyright (c) 2016 The Usdecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   usdecoincore.org and usdecoin.org and compares them.
...
WORKINGDIR="/tmp/usdecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://usdecoincore.org"
HOST2="https://usdecoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="usdecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'usdecoin-',
...
if [ $? -ne 0 ]; then
   echo "usdecoin.org failed to provide signature file, but usdecoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "usdecoin.org and usdecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Usdecoin Core binary release signing key installed?"
   fi
