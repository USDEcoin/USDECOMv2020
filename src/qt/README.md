This directory contains the UsdecoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Usdecoin UI.
...
ClientModel is used by the main application `usdecoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/usdecoin/usdecoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a usdecoin:... link from a browser).
...
* UI elements like UsdecoinAmountField, which inherit from QWidget.
* `usdecoinstrings.cpp`: automatically generated
* `usdecoinunits.(h/cpp)`: USDE / mUSDE / etc handling
* `callback.h`
...
* don't change `local/usdecoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "usdecoin-qt" under "Run", which is where you can also add command line arguments)
