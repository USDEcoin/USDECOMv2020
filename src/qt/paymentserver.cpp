...
const int USDECOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString USDECOIN_IPC_PREFIX("usdecoin:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(USDECOIN_IPC_PREFIX, Qt::CaseInsensitive)) // usdecoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseUsdecoinURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(USDECOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(USDECOIN_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("usdecoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'usdecoin://' is not a valid URI. Use 'usdecoin:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(USDECOIN_IPC_PREFIX, Qt::CaseInsensitive)) // usdecoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseUsdecoinURI(s, &recipient))
            {
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(UsdecoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
